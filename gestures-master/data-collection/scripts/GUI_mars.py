...
import subprocess
import os
#from peq1_data_collect import *
from Tkinter import *
import tkFont
#from null_gesture import *
...
class Window(Frame):
    ...
    def init_window(self):
        ...
        load = Image.open('{}/gestures/data-collection/images/mars_logo.png'.format(base_path))
        img = ImageTk.PhotoImage(load)
        
        logo = Label(self, image=img)
        logo.image = img
        logo.place(x=270,y=30)
        ...
        #wrlsButton = Button(self, text="=/=", bg="#bad6d8", command=self.connect)
        #wrlsButton.place(x=370,y=145)
    ...
    def submit_field(self, event=None):
        ...
        loop_func(self)
        self.labelLog.configure(text=log)
        #labelLog.place(x=25,y=200)
        #self.cycle_gestures()
        
        #labelinstr = Label(root, text="Start Intro Video...                            ", bg="#A7B8BF", fg="black")
        #labelinstr.place(x=45, y=260)
        #self.subButton.configure(state=DISABLED)
        #self.vidButton.configure(state=NORMAL)
    ...
    def show_image(self):
        ...
        #fnull = open(os.devnull, 'w') 
        #subprocess.call(['eog', '-f', current_image],
